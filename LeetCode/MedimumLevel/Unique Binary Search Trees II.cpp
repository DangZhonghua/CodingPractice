    ...
    if( 0 == n)
    {
      return vector<TreeNode*>();
		...
		for (int i = 1; i <= n; ++i)
		{
			...
			mapIndexTree[i][1].push_back(node);
		}
		...
		for (int L = 2; L <= n; ++L) // Create the binary from small size window
		{
			for (int i = 1; i <= (n - L + 1); ++i)
			{
				...
				for (int m = i; m <= j; ++m)  //root at node m.
				{
					...
					if (m - 1 >= i && m + 1 <= j)
					{
						auto itleft = mapIndexTree[i][LL].begin();

						for (; itleft != mapIndexTree[i][LL].end(); ++itleft)
						{
							auto itright = mapIndexTree[m + 1][RL].begin();
							for (; itright != mapIndexTree[m + 1][RL].end(); ++itright)
							{
								TreeNode* r = new TreeNode(m);
								r->left = *itleft;
								r->right = *itright;
								mapIndexTree[i][L].push_back(r);
							}
						}
					}
					else if (m - 1 >= i)
