...
the path weight is no less than K.
...
class Graph
{
	...
	int  FindLongestUtil(int src, vector<bool>& path, vector<int>& pathWeight, vector<int>& maxpathWeight);
...
public:
	...
	int  LogestPathFromSource(int src);
};
...
int  Graph::FindLongestUtil(int src, vector<bool>& path, vector<int>& pathWeight, vector<int>& maxpathWeight)
{
	int ret = 0;
	...
	for (auto it = adj[src].begin(); it != adj[src].end(); ++it)
	{
		...
		path[v] = true;
		int ow = pathWeight[v];
		pathWeight[v] = pathWeight[src] + w;
		if (maxpathWeight[v] < pathWeight[v])
		{
			maxpathWeight[v] = pathWeight[v];
		}
		FindLongestUtil(v, path, pathWeight,maxpathWeight);
		path[v] = false; //backtrack when finish the vertex traversal.  
		pathWeight[v] = ow;
	}
...
int  Graph::LogestPathFromSource(int src)
{
	int ret = 0;

	vector<bool> path(V, false);
	vector<int> pathWeight(V, INT_MIN);
	vector<int> maxpathWeight(V, INT_MIN);
	path[src] = true;
	pathWeight[src] = 0;
	FindLongestUtil(src, path, pathWeight, maxpathWeight);

	for (int w : pathWeight)
	{
		cout << w << " ";
	}
	cout << endl;
	for (int w : maxpathWeight)
	{
		cout << w << " ";
	}
...
int main()
{
	...
	g.LogestPathFromSource(src);

	return 0;
}
//Exercise: Modify the above solution to find weight of longest path from a given source.