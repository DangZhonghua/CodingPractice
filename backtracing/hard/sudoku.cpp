...
bool BtSolveSudoku(vector< vector<bool>  > & rv, vector<vector<bool>>& cv, vector<vector<bool>>& bv, vector<vector<int>> &board, int N)
{
	...
	for (int num = 1; num <= N; ++num)
	{
		if (isSafePlace(rv, cv, bv, row, col, num, N))
		{
			...
			rv[row][num] = true;
			cv[col][num] = true;
			bv[CalcGridIndex(row, col, sqrt(N))][num] = true;
			board[row][col] = num;
			if (BtSolveSudoku(rv, cv, bv, board, N))
			{
				return true;
			}
			else // this is backtrack to another choice
			{
				rv[row][num] = false;
				cv[col][num] = false;
				bv[CalcGridIndex(row, col, sqrt(N))][num] = false;
				board[row][col] = 0;
			}
		}
	}
...
int SolveSudoku(vector<vector<int>> &board, int N)
{
	...
	vector< vector<bool> > rv(N+1, vector<bool>(N+1, 0));
	vector< vector<bool> > cv(N+1, vector<bool>(N+1, 0));
	vector< vector<bool> > bv(N+1, vector<bool>(N+1, 0));
